...
const SkillsForm = ({ params }: { params: { id: string } }) => {
  ...
  return (
    <div className="p-5 shadow-lg rounded-lg border-t-primary-700 border-t-4 bg-white">
      <h2 className="text-lg font-semibold leading-none tracking-tight">Skill Sets</h2>
      <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Add your top professional key skills</p>
        ...
        {skillsList.map((item: any, index: number) => (
          ...
          >
            <div className="space-y-2 w-full">
              <label className="mt-2 text-slate-700 font-semibold">Skill Name:</label>
              <Input
            ...
            <div className="space-y-2 w-full lg:w-48">
              ...
              <select
                className="w-full px-3 py-2 border border-gray-300 rounded-md bg-white text-sm focus:outline-none focus:ring-2 focus:ring-primary-700"
                value={item.rating || 3}
                onChange={(e: any) => handleChange(index, "rating", Number.parseInt(e.target.value))}
              >
      ...
      <div className="mt-5 flex gap-2 justify-between">
        <div className="flex gap-2">
          <Button variant="outline" onClick={AddNewSkills} className="text-primary bg-transparent">
            <Plus className="size-4 mr-2" /> Add More
          </Button>
          <Button variant="outline" onClick={RemoveSkills} className="text-primary bg-transparent">
