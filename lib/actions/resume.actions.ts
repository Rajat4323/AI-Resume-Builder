...
export async function fetchResume(resumeId: string) {
  try {
    ...
    const resume = await Resume.findOne({ resumeId })
      .populate({ path: "experience", model: "Experience", options: { strictPopulate: false } })
      .populate({ path: "projects", model: "Project", options: { strictPopulate: false } })
      .populate({ path: "education", model: "Education", options: { strictPopulate: false } })
      .populate({ path: "skills", model: "Skill", options: { strictPopulate: false } })
...
export async function addExperienceToResume(resumeId: string, experienceDataArray: any) {
  try {
    const resume = await Resume.findOne({ resumeId: resumeId })
    ...
    const savedExperiences = await Promise.all(
      experienceDataArray.map(async (experienceData: any) => {
        if (experienceData._id) {
          const existingExperience = await Experience.findById(experienceData._id)
          if (existingExperience) {
            return await Experience.findByIdAndUpdate(experienceData._id, experienceData, { new: true })
          }
        }
...
export async function addProjectToResume(resumeId: string, projectDataArray: any) {
  try {
    const resume = await Resume.findOne({ resumeId: resumeId })
    ...
    const savedProjects = await Promise.all(
      projectDataArray.map(async (projectData: any) => {
        if (projectData._id) {
          ...
          if (existingProject) {
            return await Project.findByIdAndUpdate(projectData._id, projectData, { new: true })
          }
        }
...
export async function addEducationToResume(resumeId: string, educationDataArray: any) {
  try {
    const resume = await Resume.findOne({ resumeId: resumeId })
      ...
      educationDataArray.map(async (educationData: any) => {
        if (educationData._id) {
          ...
          if (existingEducation) {
            return await Education.findByIdAndUpdate(educationData._id, educationData, { new: true })
          }
        }
